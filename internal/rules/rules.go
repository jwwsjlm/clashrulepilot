...
// RenderClassical renders a standard Clash/Mihomo classical rule-provider.
// It intentionally contains only the provider payload; routing actions belong
// to the caller's RULE-SET entry in the main configuration.
func RenderClassical(s Store, action Action) []byte {
	return Render(s, action, "")
}

// RenderACL renders the plain-text rule format consumed by Subconverter rulesets.
func RenderACL(s Store, action Action) []byte {
	var b strings.Builder
	fmt.Fprintf(&b, "# Generated by ClashRulePilot. Action: %s\n# Do not edit; change data/personal_rules.json through the Bot.\n", action)
	for _, rule := range Ordered(s.Rules) {
		if rule.Action == action {
			b.WriteString(Token(rule))
			b.WriteByte('\n')
		}
	}
	return []byte(b.String())
